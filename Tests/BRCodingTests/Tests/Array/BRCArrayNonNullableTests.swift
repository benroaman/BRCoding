...
final class BRCArrayNonNullableTests: XCTestCase {
    ...
    func testDecodingCompletelyValidArray() {
        if let test = try? decoder.decode(ArrayNonNullable.self, from: ArrayTestData.completelyValidArrayJSONData) {
    ...
    func testNullLiteralDecodesToEmptyArray() {
        if let test = try? decoder.decode(ArrayNonNullable.self, from: GeneralTestData.nullLiteralJSONData) {
    ...
    func testInvalidTypeDecodesToEmptyArray() {
        if let test = try? decoder.decode(ArrayNonNullable.self, from: ArrayTestData.invalidTypeJSONData) {
    ...
    func testMissingFieldDecodesToEmptyArray() {
        if let test = try? decoder.decode(ArrayNonNullable.self, from: GeneralTestData.missingFieldJSONData) {
    ...
    func testEncodingValidArrayDecodesCorrectly() {
        let test = ArrayNonNullable(testValue: ArrayTestData.completelyValidArray)
