...
final class BRCBoolOptionalNullOmittingTests: XCTestCase {
    ...
    func testDecodeOptionalNullOmittingWithValuePresent() throws {
        ...
        if let trueStringyIntTestObject = try? decoder.decode(TestCodable.self, from: BoolTestData.trueStringyIntJSONData) {
    ...
    func testEncodeOptionalNullOmitting() {
        ...
        guard let decodedTrue = try? decoder.decode(TestCodable.self, from: encodedTrue) else { return XCTFail("Failed to decode encoded Optional Null Omitting Bool Object") }
        ...
        guard let decodedNil = try? decoder.decode(TestCodable.self, from: encodedNil) else { return XCTFail("Failed to decode encoded Optional Null Omitting Bool Object") }
